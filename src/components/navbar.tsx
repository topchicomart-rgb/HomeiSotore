...
export function Navbar() {
  ...
  return (
    <div className="sticky top-0 z-50 w-full">
      {/* Glassmorphic Navbar */}
      <nav className="glass border-b border-white/20 dark:border-slate-700/30 backdrop-blur-xl">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              {mounted && (
                <button
                  onClick={toggleTheme}
                  className="p-2 hover:bg-white/10 dark:hover:bg-slate-700/30 rounded-full transition-colors"
                >
                  {isDark ? '☀️' : '🌙'}
                </button>
              )}
              ...
              <div className="relative">
                {isLoggedIn ? (
                  <button
                    ...
                    className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 text-white flex items-center justify-center font-bold hover:shadow-lg transition-all"
                  >
                ...
                {isProfileOpen && isLoggedIn && (
                  <div className="absolute right-0 mt-2 w-48 glass rounded-2xl py-2 shadow-xl">
                    ...
                    <Link
                      href="/settings"
                      className="block px-4 py-2 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 text-sm font-medium"
                    >
                      Settings
                    </Link>
                    <button
              ...
              {isLoggedIn && (
                <button
                  ...
                  className="md:hidden p-2 hover:bg-white/10 dark:hover:bg-slate-700/30 rounded-full transition-colors"
                >
                  {isMobileMenuOpen ? (
            ...
            <div className="md:hidden pb-4">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="block px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-white/5 dark:hover:bg-slate-700/20 rounded-lg transition-colors"
